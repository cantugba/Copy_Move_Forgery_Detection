...
class DrawRectangle(AbstractShape):
    ...
    def draw(self):
        ...
        if (self.cRectangle == 0):
            ...
            cv2.rectangle(copy_image, (int(k2x[0]) + 10, int(k2n[1]) - 10), (int(k2n[0]) - 10, int(k2x[1]) + 10), self.color, 3)
            cv2.rectangle(copy_image, (int(k1x[0]) + 10, int(k1n[1]) - 10), (int(k1n[0]) - 10, int(k1x[1]) + 10), self.color, 3)
            ...
            for k1, k2 in zip(self.keypoints1, self.keypoints2):
                if len(self.keypoints1) > 1:
                    ...
                    if p == max(list):
                        new_row = [k1[0], k1[1]]
                        z2 = np.vstack([z2, new_row])
                    elif p < 0:
                        new_row = [k1[0], k1[1]]
                        z3 = np.vstack([z3, new_row])
                        new_row = [k2[0], k2[1]]
                        z4 = np.vstack([z4, new_row])
            ...
            cv2.rectangle(copy_image, (int(k2x[0]) + 10, int(k2n[1]) - 10), (int(k2n[0]) - 10, int(k2x[1]) + 10), self.color, 3)
            cv2.rectangle(copy_image, (int(k11x[0]) + 10, int(k11n[1]) - 10), (int(k11n[0]) - 10, int(k11x[1]) + 10), self.color, 3)
            cv2.rectangle(copy_image, (int(k1x[0]) + 10, int(k1n[1]) - 10), (int(k1n[0]) - 10, int(k1x[1]) + 10), self.color, 3)
